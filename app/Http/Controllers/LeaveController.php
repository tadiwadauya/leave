...
class LeaveController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->type_of_leave == 'Annual' or $request->type_of_leave == 'Cash in Lieu of Leave (CILL)' or $request->type_of_leave == 'Off Day' or $request->type_of_leave == 'Sick' or $request->type_of_leave == 'Maternity' or $request->type_of_leave == 'Compassionate') {
            ...
            $holidays = array(
                '2022-01-01', '2022-02-21', '2022-04-15', '2022-04-16', '2022-04-17', '2022-04-18', '2022-05-02', '2020-05-25', '2022-08-08', '2022-08-09',
        ...
        if ($request->type_of_leave == 'Compassionate') {
            if ($user->compassionate >= 0.5 && $user->compassionate >= $days_taken) {
                $leave = Leave::create([
            ...
            try {
                foreach ($approvers as $authorizer) {
                    ...
                    Mail::to($authorizer->email)->cc('appmailing@whelson.net.za')->send(new LeaveApproval($details));
                }
            ...
            if ($user->study_leave_days >= 0.5 && $user->study_leave_days >= $days_taken) {
                ...
                if ($request->file('studyfile')) {
                    ...
                    $filename = time() . '.' . $file->getClientOriginalExtension();
                    $request->studyfile->move('documents/', $filename);
                }
